...
.EXAMPLE
    .\setup-github-prereqs-simple.ps1 -GitHubUsername "saphirredragon207"
#>
    ...
    foreach ($prereq in $prerequisites.GetEnumerator()) {
        ...
        try {
            ...
            if (Test-Path $outputFile) {
                ...
                Write-ColorOutput "    Downloaded: $($info.filename) ($sizeMB MB)" "Green"
            } else {
...
function New-ChecksumsJson {
    ...
    $checksums = @{
        windows = @{
            vc_redist = @{
                "vc_redist.x64.exe" = @{
                    url = "https://github.com/$GitHubUsername/device-notifier-prereqs/raw/main/windows/vc_redist/vc_redist.x64.exe"
                    sha256 = ""
            ...
            dotnet = @{
                "ndp48-web.exe" = @{
                    url = "https://github.com/$GitHubUsername/device-notifier-prereqs/raw/main/windows/dotnet/ndp48-web.exe"
                    sha256 = ""
                    version = "4.8.03761"
            ...
            openssl = @{
                "Win64OpenSSL-3_0_12.exe" = @{
                    url = "https://github.com/$GitHubUsername/device-notifier-prereqs/raw/main/windows/openssl/Win64OpenSSL-3_0_12.exe"
                    sha256 = ""
                    version = "3.0.12"
...
4. Repository name: \`device-notifier-prereqs\`
...
2. Drag and drop all files from the \`github-prereqs\` folder
...
git clone https://github.com/$GitHubUsername/device-notifier-prereqs.git
cd device-notifier-prereqs

# Copy all files from the setup folder
xcopy /E /I "github-prereqs" .
...
2. Verify download URLs work:
   - https://github.com/$GitHubUsername/device-notifier-prereqs/raw/main/windows/vc_redist/vc_redist.x64.exe
   - https://github.com/$GitHubUsername/device-notifier-prereqs/raw/main/windows/dotnet/ndp48-web.exe
   - https://github.com/$GitHubUsername/device-notifier-prereqs/raw/main/windows/openssl/Win64OpenSSL-3_0_12.exe
...
## Repository URL
https://github.com/$GitHubUsername/device-notifier-prereqs
"@
...
function Main {
    ...
    Write-ColorOutput "Repository Name: device-notifier-prereqs" "Cyan"
    Write-ColorOutput "Output Path: .\github-prereqs" "Cyan"
    Write-Host ""

    # Set output path
    $script:OutputPath = ".\github-prereqs"
    ...
    Write-ColorOutput "  https://github.com/$GitHubUsername/device-notifier-prereqs" "Yellow"
    Write-Host ""
